...
{
	uintptr_t node = Driver::read_memory<uintptr_t>(entityPawn + Offsets::m_pGameSceneNode); // m_pGameSceneNode
	uintptr_t boneMatrix = Driver::read_memory<uintptr_t>(node + 0x1F0); // m_groundNormal 
	return boneMatrix;
}
